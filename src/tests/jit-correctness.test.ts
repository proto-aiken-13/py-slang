...
function runJIT(code: string): unknown {
  const src = code.endsWith("\n") ? code : code + "\n";
  const ast = parse(src);
  const compiler = SVMLCompiler.fromProgram(ast);
  const program = compiler.compileProgram(ast);
...
const CASES: Array<{
  ...
  {
    name: "if pos>0: takes true branch",
    code: "def f(x):\n  if x > 0:\n    return 1\n  else:\n    return 2\nf(5)",
    expected: 1,
  },
  {
    name: "if neg>0: takes false branch",
    code: "def f(x):\n  if x > 0:\n    return 1\n  else:\n    return 2\nf(-5)",
    expected: 2,
  },
  {
    name: "if zero>0: takes false branch",
    code: "def f(x):\n  if x > 0:\n    return 1\n  else:\n    return 2\nf(0)",
    expected: 2,
  },
  {
...
bt(1,-1,1)+bt(1,-1,0)+bt(1,0,1)+bt(1,0,0)+bt(0,-1,1)+bt(0,-1,0)+bt(0,0,1)+bt(0,0,0)`,
  ...
  { name: "x + 0 = x (zero identity)", code: "def f(x,y):\n  return x + y\nf(42,0)", expected: 42 },
  { name: "0 + x = x (zero identity)", code: "def f(x,y):\n  return x + y\nf(0,42)", expected: 42 },
  { name: "x - 0 = x", code: "def f(x,y):\n  return x - y\nf(42,0)", expected: 42 },
  { name: "x * 0 = 0", code: "def f(x,y):\n  return x * y\nf(42,0)", expected: 0 },
  { name: "0 * x = 0", code: "def f(x,y):\n  return x * y\nf(0,42)", expected: 0 },
  ...
  {
    name: "bool NOT int zero (fallback to generic)",
    code: "def f(x):\n  return not x\nf(0)",
    expected: true,
    skipNoJit: true,
  },
