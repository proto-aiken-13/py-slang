...
import { positiveInteger, negativeInteger, zeroInteger, trueValue, falseValue, stringValue } from "../types/lattice-ops";
...
export class SVMLInterpreter {
  ...
  private functions: readonly SVMLIR[];
  ...
  private typeObservations: Map<number, Map<number, AbstractValue>[]> = new Map();

  private compiler: SVMLCompiler | null = null;
  ...
  reset(program: SVMLProgram, instrumentation?: InstrumentationTracker, compiler?: SVMLCompiler): void {
    const programChanged = program !== this.program;
    this.program = program;
    this.functions = program.functions;
    this.instrumentation = instrumentation || null;
    this.currentFrame = null;
    ...
    if (programChanged) this.sigCaches = [];
  }
  ...
  private run(): SVMLBoxType {
    while (!this.halted && this.currentFrame) {
      ...
      __DEBUG__ && debug(`PC=${pc} | ${OpCodes[op] || `UNKNOWN(${op})`} ${a1} ${a2} | Stack: [${frame.stack.map((v) => JSON.stringify(SVMLInterpreter.toJSValue(v))).join(", ")}]`);
      ...
      switch (op) {
        ...
        case OpCodes.MODF: {
          ...
          this.push(((left % right) + right) % right);
          break;
        }
        // Unary operations
        case OpCodes.NEGG: {
        ...
        case OpCodes.NEWITER: {
          ...
          if (typeof iterable === "object" && iterable !== null) {
            if ((iterable as any).type === "array") {
              const iter: SVMLIterator = {
                type: "iterator",
                kind: "list",
                array: iterable as SVMLArray,
                index: 0,
              };
              this.push(iter);
            } else if ((iterable as any).type === "iterator") {
        ...
        case OpCodes.FOR_ITER: {
          ...
          if (iter.kind === "range") {
            const going = iter.step! > 0 ? iter.current! < iter.stop!
        ...
        default:
          throw new Error(
            `Unimplemented opcode: ${op} (${OpCodes[op] || "UNKNOWN"})`
          );
      }
    }
  ...
  private pop(): SVMLBoxType {
    ...
    if (this.currentFrame.stack.length === 0) {
      __DEBUG__  && debug(`STACK UNDERFLOW! Current frame: ${this.currentFrame.closure.functionIndex}`);
      throw new Error("Stack underflow");
    }
    const value = this.currentFrame.stack.pop()!;
    __DEBUG__  && debug(`  Popped: ${JSON.stringify(SVMLInterpreter.toJSValue(value))}`);
    return value;
  }
  ...
  private storeLocal(slot: number): void {
    ...
    __DEBUG__  && debug(`[STLG] Storing to slot ${slot}: ${JSON.stringify(SVMLInterpreter.toJSValue(value))}`);
  ...
  private loadParent(slot: number, level: number): void {
    ...
    __DEBUG__  && debug(`[LDPG] Loading from parent env: slot=${slot}, level=${level}`);

    try {
      const parentEnv = this.currentFrame.env.getParent(level);
      __DEBUG__  && debug(`[LDPG] Parent env has ${parentEnv.getSize()} slots`);
      const value = parentEnv.get(slot);
      __DEBUG__  && debug(`[LDPG] Loaded value: ${JSON.stringify(SVMLInterpreter.toJSValue(value))}`);
      this.push(value);
    } catch (e: any) {
      __DEBUG__  && debug(`[LDPG] ERROR: ${e.message}`);
      throw e;
    }
  }
  ...
  private computeSigKey(args: SVMLBoxType[]): number {
    ...
    for (let i = 0; i < args.length; i++) {
      const v = args[i];
      const t = typeof v;
      let tag = SigTag.Unknown;
      if (t === "number") {
        tag = (v as number) > 0 ? SigTag.Pos : (v as number) < 0 ? SigTag.Neg : SigTag.Zero;
        hasKnown = true;
      } else if (t === "boolean") {
        tag = v ? SigTag.BTrue : SigTag.BFalse;
        hasKnown = true;
      } else if (t === "string") {
        tag = SigTag.Str;
        hasKnown = true;
  ...
  private getSpecializedIR(funcDef: SVMLIR, functionIndex: number, args: SVMLBoxType[], closure: SVMLClosure): SVMLIR {
    ...
    // Warm path: check per-function cache
    let fnCache = this.sigCaches[functionIndex];
    if (fnCache) {
      const cached = fnCache.get(sigKey);
      if (cached) {
        closure.lastSig = sigKey;
        closure.lastIR = cached;
        return cached;
      }
    }

    // Miss path: specialize
    const funcAST = this.program.functionASTs.get(functionIndex);
    const compiler = this.compiler;
    if (!funcAST || !compiler) return funcDef;

    const funcNode = funcAST as StmtNS.FunctionDef | ExprNS.Lambda | ExprNS.MultiLambda;

    // Build bindings from runtime args
    const bindings = new Map<number, AbstractValue>();
    for (let i = 0; i < args.length; i++) {
      const t = typeof args[i];
      if (t === "number") {
        const n = args[i] as number;
        bindings.set(i, n > 0 ? positiveInteger() : n < 0 ? negativeInteger() : zeroInteger());
      } else if (t === "boolean") {
        bindings.set(i, args[i] ? trueValue() : falseValue());
      } else if (t === "string") {
        bindings.set(i, stringValue());
      }
    }

    // Record this profile for collectTypeInfo() consumers.
    // bindings keys 0..args.length-1 are parameter positions.
    if (bindings.size > 0) {
      let profiles = this.typeObservations.get(functionIndex);
      if (!profiles) {
        profiles = [];
        this.typeObservations.set(functionIndex, profiles);
      }
      profiles.push(new Map(bindings));
    }

    // Lambda body (with cached wrapper)
    let body: StmtNS.Stmt[];
    if (funcNode instanceof ExprNS.Lambda) {
      if (!(funcNode as any)._cachedReturnWrapper) {
        (funcNode as any)._cachedReturnWrapper = [
          new StmtNS.Return(funcNode.startToken, funcNode.endToken, funcNode.body)
        ];
      }
      body = (funcNode as any)._cachedReturnWrapper;
    } else {
      body = funcNode.body;
    }
  ...
  /**
   * Returns the raw type observations accumulated during this execution.
   * Maps functionIndex → list of observed profiles (paramIndex → AbstractValue).
   * Called by SVMLBackend.collectTypeInfo() after execute() completes.
   */
  getTypeObservations(): ReadonlyMap<number, ReadonlyMap<number, AbstractValue>[]> {
  ...
  private call(numArgs: number, isTailCall: boolean): void {
    ...
    __DEBUG__  && debug(`[CALL] numArgs=${numArgs}, stackSize=${this.currentFrame.stack}, isTail=${isTailCall}`);
    ...
    __DEBUG__  && debug(`[CALL] Popped ${numArgs} args, stack now has ${this.currentFrame.stack.length} items`);
    ...
    const func = this.pop();
    __DEBUG__  && debug(`[CALL] Popped function: ${JSON.stringify(SVMLInterpreter.toJSValue(func))}`);
    ...
    if (numArgs !== funcDef.numArgs) {
      throw new Error(
        `Function expects ${funcDef.numArgs} arguments but got ${numArgs}`
      );
    }

    const specializedIR = this.getSpecializedIR(funcDef, closure.functionIndex, args, closure);
    ...
    for (let i = 0; i < numArgs; i++) {
      newEnv.set(i, args[i]);
      __DEBUG__  && debug(`[CALL] Set env slot ${i} = ${JSON.stringify(SVMLInterpreter.toJSValue(args[i]))}`);
    }

    __DEBUG__  && debug(`[CALL] Created new env with ${funcDef.envSize} slots, parent exists: ${closure.parentEnv !== null}`);

    if (isTailCall) {
      // Tail call optimization: reuse current frame
      this.currentFrame.closure = closure;
      this.currentFrame.ir = specializedIR;
  ...
  private callPrimitive(
    ...
    __DEBUG__  && debug(`[CALLP] primitiveIndex=${primitiveIndex}, numArgs=${numArgs}`);
    ...
    __DEBUG__  && debug(`[CALLP] Calling primitive ${primitiveIndex} with args: ${JSON.stringify(args.map((a) => SVMLInterpreter.toJSValue(a)))}`);
    ...
    __DEBUG__  && debug(`[CALLP] Primitive returned: ${JSON.stringify(SVMLInterpreter.toJSValue(result))}`);
  }
  ...
  private return(): void {
    ...
    __DEBUG__  && debug(`[RETG] Returning value: ${JSON.stringify(SVMLInterpreter.toJSValue(returnValue))}`);
    ...
    __DEBUG__  && debug(`[RETG] Pushed return value to caller's stack, size now: ${this.currentFrame.stack.length}`);
  }
