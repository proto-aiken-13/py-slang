...
import type { SVMLBoxType, SVMLClosure, SVMLArray } from "./types";
import {
  positiveInteger, negativeInteger, zeroInteger,
  trueValue, falseValue,
  stringValue, closureValue, nullValue, TOP,
...
export class SVMLAdapter implements BackendAdapter<SVMLBoxType> {
  toPython(native: SVMLBoxType): PythonType {
    ...
    switch (typeof native) {
      case "number": return { tag: "int", value: native };
      case "boolean": return { tag: "bool", value: native };
      case "string": return { tag: "str", value: native };
      default:
        if (typeof native === "object" && native !== null && "type" in native) {
          ...
          if (native.type === "array") {
            return { tag: "list", elements: (native as SVMLArray).elements.map(e => this.toPython(e)) };
          }
        }
        return { tag: "none" };
    }
  ...
  toAbstractValue(python: PythonType): AbstractValue {
    switch (python.tag) {
      ...
      case "bool": return python.value ? trueValue() : falseValue();
      case "str": return stringValue();
      case "none": return nullValue();
      case "function": return closureValue();
      case "list": return TOP;
    }
  }
}
