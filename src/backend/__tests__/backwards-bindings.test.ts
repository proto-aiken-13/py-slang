...
describe("BackwardsBindings", () => {
  it("resolves registered nodes from numeric observations", () => {
    ...
    const profile0 = new Map([[0, {} as any]]);
    const profile1 = new Map([[0, {} as any], [1, {} as any]]);
    const obs: ReadonlyMap<number, ReadonlyMap<number, any>[]> = new Map([
      [0, [profile0]],
      [1, [profile1, profile1]],
    ]);
  ...
  it("works with string IDs (generic ID = string variant)", () => {
    ...
    const obs: ReadonlyMap<string, ReadonlyMap<number, any>[]> = new Map([
      ["my_func", [new Map([[0, {} as any]])]],
    ]);
    const result = b.resolve(obs);
    expect(result.size).toBe(1);
    expect(result.has(n)).toBe(true);
  ...
  it("resolve() can be called multiple times on the same bindings", () => {
    ...
    const obs1: ReadonlyMap<number, ReadonlyMap<number, any>[]> = new Map([[3, [new Map()]]]);
    const obs2: ReadonlyMap<number, ReadonlyMap<number, any>[]> = new Map([[3, [new Map(), new Map()]]]);
