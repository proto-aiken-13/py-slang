 ...
 */
function formatSVMLProgram(program: SVMLProgram): string {
...
function compilePythonToSVML(pythonCode: string, outputFile: string, format: string) {
  try {
    ...
    console.log(
      `  Total Instructions: ${program.functions.reduce((total, func) => total + func.count, 0)}`,
    );
...
function main() {
  ...
  program
    ...
    .action((inputFile: string, format: string, options: { output?: string }) => {
      if (!fs.existsSync(inputFile)) {
        console.error(`Error: File '${inputFile}' not found`);
        process.exit(1);
      }
      const outputFile =
        options.output || inputFile.replace(/\.py$/, format === "text" ? ".txt" : ".svm");
